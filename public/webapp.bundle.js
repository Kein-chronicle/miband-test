(function () {
            ...
            var _require = /*#__PURE__*/Object.freeze({
            ...
            SafeBuffer.allocUnsafeSlow = function (size) {
              ...
              return _require.SlowBuffer(size)
            };
            });
            ...
            var Buffer$a = _require.Buffer;

            var inspect$1 = _require2.inspect;
            ...
            function CorkedRequest$1(state$$1) {
              var _this = this;
            ...
            function validChunk$1(stream, state$$1, chunk, cb) {
              var er;
            ...
            Writable$1.prototype.write = function (chunk, encoding, cb) {
              ...
              if (state$$1.ending) writeAfterEnd$1(this, cb);else if (isBuf || validChunk$1(this, state$$1, chunk, cb)) {
                state$$1.pendingcb++;
                ret = writeOrBuffer$1(this, state$$1, isBuf, chunk, encoding, cb);
              }
              return ret;
            };
            ...
            Writable$1.prototype.uncork = function () {
              ...
              if (state$$1.corked) {
                state$$1.corked--;
                if (!state$$1.writing && !state$$1.corked && !state$$1.bufferProcessing && state$$1.bufferedRequest) clearBuffer$1(this, state$$1);
              }
            };
            ...
            function decodeChunk$1(state$$1, chunk, encoding) {
            ...
            function writeOrBuffer$1(stream, state$$1, isBuf, chunk, encoding, cb) {
              if (!isBuf) {
                var newChunk = decodeChunk$1(state$$1, chunk, encoding);
              ...
              if (state$$1.writing || state$$1.corked) {
                ...
                doWrite$1(stream, state$$1, false, len, chunk, encoding, cb);
              }

              return ret;
            }

            function doWrite$1(stream, state$$1, writev, len, chunk, encoding, cb) {
              state$$1.writelen = len;
              state$$1.writecb = cb;
              state$$1.writing = true;
              state$$1.sync = true;
              if (state$$1.destroyed) state$$1.onwrite(new ERR_STREAM_DESTROYED('write'));else if (writev) stream._writev(chunk, state$$1.onwrite);else stream._write(chunk, encoding, state$$1.onwrite);
              state$$1.sync = false;
            }

            function onwriteError$1(stream, state$$1, sync, er, cb) {
              --state$$1.pendingcb;
              ...
              if (sync) {
                ...
                nextTick(finishMaybe$1, stream, state$$1);
                stream._writableState.errorEmitted = true;
                errorOrDestroy$1(stream, er);
              } else {
                ...
                finishMaybe$1(stream, state$$1);
              }
            }

            function onwriteStateUpdate$1(state$$1) {
            ...
            function onwrite$1(stream, er) {
              var state$$1 = stream._writableState;
              var sync = state$$1.sync;
              var cb = state$$1.writecb;
              if (typeof cb !== 'function') throw new ERR_MULTIPLE_CALLBACK();
              onwriteStateUpdate$1(state$$1);
              if (er) onwriteError$1(stream, state$$1, sync, er, cb);else {
                // Check if we're actually ready to finish, but don't emit yet
                var finished = needFinish$1(state$$1) || stream.destroyed;

                if (!finished && !state$$1.corked && !state$$1.bufferProcessing && state$$1.bufferedRequest) {
                  clearBuffer$1(stream, state$$1);
                }

                if (sync) {
                  nextTick(afterWrite$1, stream, state$$1, finished, cb);
                } else {
                  afterWrite$1(stream, state$$1, finished, cb);
                }
              }
            }

            function afterWrite$1(stream, state$$1, finished, cb) {
              if (!finished) onwriteDrain$1(stream, state$$1);
              state$$1.pendingcb--;
              cb();
              finishMaybe$1(stream, state$$1);
            } // Must force callback to be called on nextTick, so that we don't
            ...
            function onwriteDrain$1(stream, state$$1) {
            ...
            function clearBuffer$1(stream, state$$1) {
              ...
              if (stream._writev && entry && entry.next) {
                ...
                doWrite$1(stream, state$$1, true, state$$1.length, buffer, '', holder.finish); // doWrite is almost always async, defer these to save a bit of time
                // as the hot path ends with doWrite
                ...
                if (holder.next) {
                  ...
                  state$$1.corkedRequestsFree = new CorkedRequest$1(state$$1);
                }
            ...
            Writable$1.prototype.end = function (chunk, encoding, cb) {
              ...
              if (!state$$1.ending) endWritable$1(this, state$$1, cb);
              return this;
            };
            ...
            function prefinish$1(stream, state$$1) {
              if (!state$$1.prefinished && !state$$1.finalCalled) {
                if (typeof stream._final === 'function' && !state$$1.destroyed) {
                  state$$1.pendingcb++;
                  state$$1.finalCalled = true;
                  nextTick(callFinal, stream, state$$1);
                } else {
                  state$$1.prefinished = true;
                  stream.emit('prefinish');
                }
            ...
            function finishMaybe$1(stream, state$$1) {
              var need = needFinish$1(state$$1);

              if (need) {
                prefinish$1(stream, state$$1);
            ...
            function endWritable$1(stream, state$$1, cb) {
              state$$1.ending = true;
              finishMaybe$1(stream, state$$1);

              if (cb) {
                if (state$$1.finished) nextTick(cb);else stream.once('finish', cb);
              }
            ...
            if (_require2 && _require2.debuglog) {
              debug$1 = _require2.debuglog('stream');
            } else {
            ...
            function readableAddChunk$1(stream, chunk, encoding, addToFront, skipChunkCheck) {
              ...
              if (chunk === null) {
                ...
                onEofChunk$1(stream, state$$1);
              } else {
                var er;
                if (!skipChunkCheck) er = chunkInvalid$1(state$$1, chunk);

                if (er) {
                  errorOrDestroy$2(stream, er);
                } else if (state$$1.objectMode || chunk && chunk.length > 0) {
                  if (typeof chunk !== 'string' && !state$$1.objectMode && Object.getPrototypeOf(chunk) !== Buffer$d.prototype) {
                    chunk = _uint8ArrayToBuffer$1(chunk);
                  }

                  if (addToFront) {
                    if (state$$1.endEmitted) errorOrDestroy$2(stream, new ERR_STREAM_UNSHIFT_AFTER_END_EVENT());else addChunk(stream, state$$1, chunk, true);
                    ...
                    if (state$$1.decoder && !encoding) {
                      ...
                      if (state$$1.objectMode || chunk.length !== 0) addChunk(stream, state$$1, chunk, false);else maybeReadMore$1(stream, state$$1);
                    } else {
                      addChunk(stream, state$$1, chunk, false);
                    }
                  }
                } else if (!addToFront) {
                  state$$1.reading = false;
                  maybeReadMore$1(stream, state$$1);
                }
            ...
            function addChunk(stream, state$$1, chunk, addToFront) {
              if (state$$1.flowing && state$$1.length === 0 && !state$$1.sync) {
                ...
                if (state$$1.needReadable) emitReadable$1(stream);
              }

              maybeReadMore$1(stream, state$$1);
            }

            function chunkInvalid$1(state$$1, chunk) {
              var er;
            ...
            function howMuchToRead$1(n, state$$1) {
              ...
              if (n > state$$1.highWaterMark) state$$1.highWaterMark = computeNewHighWaterMark$1(n);
            ...
            Readable$1.prototype.read = function (n) {
              ...
              if (n === 0 && state$$1.needReadable && ((state$$1.highWaterMark !== 0 ? state$$1.length >= state$$1.highWaterMark : state$$1.length > 0) || state$$1.ended)) {
                debug$1('read: emitReadable', state$$1.length, state$$1.ended);
                if (state$$1.length === 0 && state$$1.ended) endReadable$1(this);else emitReadable$1(this);
                return null;
              }

              n = howMuchToRead$1(n, state$$1); // if we've ended, and we're now clear, then finish it up.

              if (n === 0 && state$$1.ended) {
                if (state$$1.length === 0) endReadable$1(this);
                return null;
              } // All the actual chunk generation logic needs to be
              // *below* the call to _read.  The reason is that in certain
              // synthetic stream cases, such as passthrough streams, _read
              ...
              if (state$$1.ended || state$$1.reading) {
                ...
                if (!state$$1.reading) n = howMuchToRead$1(nOrig, state$$1);
              }

              var ret;
              if (n > 0) ret = fromList$1(n, state$$1);else ret = null;
              ...
              if (state$$1.length === 0) {
                ...
                if (nOrig !== n && state$$1.ended) endReadable$1(this);
              }
            ...
            function onEofChunk$1(stream, state$$1) {
              debug$1('onEofChunk');
            ...
            function emitReadable$1(stream) {
              var state$$1 = stream._readableState;
              debug$1('emitReadable', state$$1.needReadable, state$$1.emittedReadable);
              state$$1.needReadable = false;

              if (!state$$1.emittedReadable) {
                debug$1('emitReadable', state$$1.flowing);
                state$$1.emittedReadable = true;
            ...
            function emitReadable_$1(stream) {
              var state$$1 = stream._readableState;
              debug$1('emitReadable_', state$$1.destroyed, state$$1.length, state$$1.ended);
            ...
            function maybeReadMore$1(stream, state$$1) {
              if (!state$$1.readingMore) {
                state$$1.readingMore = true;
                nextTick(maybeReadMore_$1, stream, state$$1);
              }
            }

            function maybeReadMore_$1(stream, state$$1) {
            ...
            Readable$1.prototype.pipe = function (dest, pipeOpts) {
              ...
              debug$1('pipe count=%d opts=%j', state$$1.pipesCount, pipeOpts);
              var doEnd = (!pipeOpts || pipeOpts.end !== false) && dest !== process.stdout && dest !== process.stderr;
              var endFn = doEnd ? onend : unpipe;
              if (state$$1.endEmitted) nextTick(endFn);else src.once('end', endFn);
              dest.on('unpipe', onunpipe);
              ...
              function ondata(chunk) {
                ...
                if (ret === false) {
                  ...
                  // also returned false.
                  // => Check whether `dest` is still a piping destination.
                  if ((state$$1.pipesCount === 1 && state$$1.pipes === dest || state$$1.pipesCount > 1 && indexOf$1(state$$1.pipes, dest) !== -1) && !cleanedUp) {
                    debug$1('false write response, pause', state$$1.awaitDrain);
                    state$$1.awaitDrain++;
                  }
            ...
            function pipeOnDrain$1(src) {
              return function pipeOnDrainFunctionResult() {
                ...
                debug$1('pipeOnDrain', state$$1.awaitDrain);
            ...
            Readable$1.prototype.unpipe = function (dest) {
              ...
              var index = indexOf$1(state$$1.pipes, dest);
            ...
            Readable$1.prototype.on = function (ev, fn) {
              ...
              if (ev === 'data') {
                ...
                if (!state$$1.endEmitted && !state$$1.readableListening) {
                  ...
                  state$$1.flowing = false;
                  state$$1.emittedReadable = false;
                  debug$1('on readable', state$$1.length, state$$1.reading);
            ...
            function resume$1(stream, state$$1) {
              if (!state$$1.resumeScheduled) {
                state$$1.resumeScheduled = true;
                nextTick(resume_$1, stream, state$$1);
              }
            }

            function resume_$1(stream, state$$1) {
              debug$1('resume', state$$1.reading);
              ...
              stream.emit('resume');
              flow$1(stream);
              if (state$$1.flowing && !state$$1.reading) stream.read(0);
            }
            ...
            function flow$1(stream) {
              var state$$1 = stream._readableState;
              debug$1('flow', state$$1.flowing);

              while (state$$1.flowing && stream.read() !== null) {
              }
            } // wrap an old-style stream as the async data source.
            // This is *not* part of the readable stream interface.
            // It is an ugly unfortunate mess of history.
            ...
            function endReadable$1(stream) {
              var state$$1 = stream._readableState;
              debug$1('endReadable', state$$1.endEmitted);

              if (!state$$1.endEmitted) {
                state$$1.ended = true;
                nextTick(endReadableNT$1, state$$1, stream);
              }
            }

            function endReadableNT$1(state$$1, stream) {
              debug$1('endReadableNT', state$$1.endEmitted, state$$1.length); // Check that we didn't get one last unshift.
            ...
            Cipher.prototype._update = function (data) {
              ...
              return Buffer$h.concat(out)
            };

            var PADDING = Buffer$h.alloc(16, 0x10);
            ...
            Splitter.prototype.add = function (data) {
              this.cache = Buffer$h.concat([this.cache, data]);
            };
            ...
            Splitter.prototype.flush = function () {
              ...
              var padBuff = Buffer$h.allocUnsafe(len);
              ...
              return Buffer$h.concat([this.cache, padBuff])
            };
            ...
            function createCipheriv (suite, password, iv) {
              ...
              if (typeof password === 'string') password = Buffer$h.from(password);
              if (password.length !== config.key / 8) throw new TypeError('invalid key length ' + password.length)

              if (typeof iv === 'string') iv = Buffer$h.from(iv);
            ...
            var browser_1 = browser$2.createCipher;
            var browser_2 = browser$2.Cipher;
            var browser_3 = browser$2.createCipheriv;
            var browser_4 = browser$2.Cipheriv;
            var browser_5 = browser$2.createDecipher;
            var browser_6 = browser$2.Decipher;
            var browser_7 = browser$2.createDecipheriv;
            var browser_8 = browser$2.Decipheriv;
            var browser_9 = browser$2.listCiphers;
            var browser_10 = browser$2.getCiphers;
            ...
            var browser_1$1 = browser$3.formatArgs;
            var browser_2$1 = browser$3.save;
            var browser_3$1 = browser$3.load;
            var browser_4$1 = browser$3.useColors;
            var browser_5$1 = browser$3.storage;
            var browser_6$1 = browser$3.destroy;
            var browser_7$1 = browser$3.colors;
            var browser_8$1 = browser$3.log;

            const debug$2 = browser$3('MiBand');
              ...
              _handleNotify(event) {
                ...
                if (event.target.uuid === this.char.auth.uuid) {
                  ...
                  if (cmd === '100101') {         // Set New Key OK
                    ...
                    let cipher = browser$2.createCipheriv('aes-128-ecb', this.key, '').setAutoPadding(false);
            ...
            async function scan() {
              ...
              try {
                ...
                log$1('Notifications demo...');
                await miband$$1.showNotification('message');
